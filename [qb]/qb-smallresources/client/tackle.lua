...
RegisterCommand('tackle', function()
    ...
    local ped = PlayerPedId()
    if distance ~= -1 and distance < 2 and GetEntitySpeed(ped) > 2.5 and not IsPedInAnyVehicle(ped, false) and not QBCore.Functions.GetPlayerData().metadata["ishandcuffed"] and not IsPedRagdoll(ped) and QBCore.Functions.GetPlayerData().job.name == 'police' then
