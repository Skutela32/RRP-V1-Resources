...
AddEventHandler('qb-communityservice:inCommunityService', function(actions_remaining)
	...
	while actionsRemaining > 0 and communityServiceFinished ~= true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), Config.ServiceLocation.x, Config.ServiceLocation.y, Config.ServiceLocation.z) > 70 then
			Teleport(PlayerPedId(), Config.ServiceLocation)
...
function ApplyPrisonerSkin()
	...
	if DoesEntityExist(playerPed) then
		...
		local DataFemale = {
				outfitData = {
			['t-shirt'] = {item = 60,texture = 1},
			['torso2']  ={item = 233,texture = 0},
			['decals'] = {item = 0, texutre = 0},
			['arms']     = {item = 46, texture = 0},
			['pants']  ={item = 247, texture = 0},   
			['shoes']  = {item = 125, texture = 1},
			['vest']  = {item = 0, texture = 0},
			['bag']  = {item = 0, texture = 0},
			['mask']  = {item = 0, texture = 0},
			['hat']  = {item = 0, texture = 0},
