...
-- text display thread
local postalHUDVisible = false -- Variable to track Postal HUD visibility
...
Citizen.CreateThread(function()
    ...
    while true do
        if nearest and not IsHudHidden() and postalHUDVisible then
...
TriggerEvent('chat:addSuggestion', '/postalhud', 'To Enable / Disable Postal HUD',
             { { name = 'Postal HUD', help = 'To Enable / Disable Postal HUD' } })


-- Command to toggle Postal HUD on/off
RegisterCommand("postalhud", function()
    postalHUDVisible = not postalHUDVisible
    if postalHUDVisible then
        TriggerEvent('QBCore:Notify', ('Postal Map is Now Active!'), 'success')
    else
        TriggerEvent('QBCore:Notify', ('Postal Map is Now Disabled'), 'error')
    end
end, false)
