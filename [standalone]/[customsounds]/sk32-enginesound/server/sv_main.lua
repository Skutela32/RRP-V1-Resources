    ...
    if balance >= price then
      if moneyType == 'cash' then
        ...
        if Config.Settings['Payments']['RenewedBanking'] then
          exports['Renewed-Banking']:handleTransaction(Player.PlayerData.citizenid, "Engine Swap", price, "Swapped Engine by Mechanics", "Mechanic Shop", "" .. Player.PlayerData.charinfo.firstname .. " " .. Player.PlayerData.charinfo.lastname .. "", "withdraw")
...
CreateThread(function()
  local ret = SqlFunc(Config.Settings['sql'],'fetchAll','SELECT * FROM sk32_enginesound', {})
...
function SaveExhaust(plate, exhaust)
    ...
    local result = SqlFunc(Config.Settings['sql'],'fetchAll','SELECT * FROM sk32_enginesound WHERE TRIM(plate) = @plate', {['@plate'] = plate_})
    if result[1] == nil then
      SqlFunc(Config.Settings['sql'],'execute','INSERT INTO sk32_enginesound (plate, exhaust) VALUES (@plate, @engine)', {
          ['@plate']   = plate,
          ['@engine']   = exhaust,
      })
    elseif result[1] then
      SqlFunc(Config.Settings['sql'],'execute','UPDATE sk32_enginesound SET exhaust = @engine WHERE TRIM(plate) = @plate', {
          ['@plate'] = plate_,
          ['@engine'] = exhaust,
      })
    end
