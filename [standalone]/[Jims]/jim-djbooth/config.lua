...
Config = {
	...
	Locations = {
		...
		{ -- Gabz uwu Cafe
			...
			DefaultVolume = 0.2,
			radius = 30,
			coords = vec3(-585.28, -1055.92, 22.34),
			prop = `prop_radio_01`	-- Prop to spawn at location, if the location doesn't have one already
		},
		{ -- Gabz Bahama Mamas
			job = "bahama",
			...
			DefaultVolume = 0.2,
			radius = 35,
			coords = vec3(-1376.87, -607.72, 31.32),
			prop = `prop_radio_01`	-- Prop to spawn at location, if the location doesn't have one already
		},
		...
		{ -- LostMC compound next to Casino
			...
			DefaultVolume = 0.1,
			radius = 20,
			coords = vec3(382.66, -935.6, 29.41),
			soundLoc = vec3(382.66, -935.6, 29.41), -- Add sound origin location if you don't want the music to play from the dj booth
		},
	},
}
