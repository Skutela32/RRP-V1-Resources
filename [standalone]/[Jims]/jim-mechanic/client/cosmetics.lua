...
RegisterNetEvent('jim-mechanic:client:Cosmetic:Check', function(part) local validMods, Menu, vehicle, Ped = {}, {}, nil, PlayerPedId()
	...
	if DoesEntityExist(vehicle) then
        if Config.System.Menu == "qb" then
            Menu[#Menu+1] = { icon = part, header = searchCar(vehicle), txt = Loc[Config.Lan]["check"].label10, isMenuHeader = true }
		...
		if Config.System.Menu == "ox" and countTable(Menu) ~= 0 then exports.ox_lib:registerContext({id = 'Menu', title = searchCar(vehicle), position = 'top-right', options = Menu }) exports.ox_lib:showContext("Menu")
...
RegisterNetEvent('jim-mechanic:client:Cosmetic:Choose', function(data) local validMods, Menu, vehicle, Ped = {}, {}, nil, PlayerPedId()
	...
	if DoesEntityExist(vehicle) then
		...
		Menu[#Menu+1] = {
			...
			header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Cosmetic:Check", args = data.part },
			title = Loc[Config.Lan]["common"].ret, event = "jim-mechanic:client:Cosmetic:Check", args = data.part
		}
        if not data.horn and not data.extra then
            Menu[#Menu+1] = {
                icon = icon, isMenuHeader = stockDisabled, disabled = (Config.System.Menu == "ox" and stockDisabled),
                header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Cosmetic:Apply",
                args = { mod = -1, id = data.id, part = data.part, plate = data.plate, window = data.window, oldLiv = data.oldLiv, roofLiv = data.roofLiv, extra = data.extra }},
                title = "0 - "..Loc[Config.Lan]["common"].stock, description = stockinstall, event = "jim-mechanic:client:Cosmetic:Apply",
                args = { mod = -1, id = data.id, part = data.part, plate = data.plate, window = data.window, oldLiv = data.oldLiv, roofLiv = data.roofLiv, extra = data.extra },
            }
        end
        if data.horn then
            data.veh = vehicle
            Menu[#Menu+1] = {
                header = "Test Horn", params = { event = "jim-mechanic:client:Horn:Test", args = data, },
                title = "Test Horn", event = "jim-mechanic:client:Horn:Test", args = data,
            }
        end
		for l, b in pairs(validMods) do
			Menu[#Menu+1] = {
				...
				params = { event = "jim-mechanic:client:Cosmetic:Apply", args = b },
...
RegisterNetEvent('jim-mechanic:client:Tires:Check', function() local Ped = PlayerPedId()
    ...
    if DoesEntityExist(vehicle) then
        ...
        if Config.System.Menu ~= "ox" then
            Menu[#Menu+1] = { header = header, txt = Loc[Config.Lan]["smoke"].menuheader, isMenuHeader = true, }
            Menu[#Menu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan]["common"].close, params = { event = "jim-mechanic:client:Menu:Close" } }
        end
		Menu[#Menu+1] = { icon = "fas fa-palette",
            header = Loc[Config.Lan]["smoke"].custom, params = { event = "jim-mechanic:client:smokeCustomMenu", },
            title = Loc[Config.Lan]["smoke"].custom, event = "jim-mechanic:client:smokeCustomMenu",
        }
		Menu[#Menu+1] = { icon = "fas fa-eraser",
            ...
            params = { event = "jim-mechanic:client:Tires:Apply", args = { R = 0, G = 0, B = 0 } },
            title = Loc[Config.Lan]["smoke"].remove, description = ((r == 0) and (g == 0) and (b == 0)) and Loc[Config.Lan]["common"].current,
            event = "jim-mechanic:client:Tires:Apply", args = { R = 0, G = 0, B = 0 },
        }
        for k, v in pairs(smokeTable) do
            Menu[#Menu+1] = {
                header = v.header,
                txt = ((r == v.r) and (g == v.g) and (b == v.b)) and Loc[Config.Lan]["common"].current,
                params = { event = "jim-mechanic:client:Tires:Apply", args = { R = v.r, G = v.g, B = v.b } },
                title = v.header,
                description = ((r == v.r) and (g == v.g) and (b == v.b)) and Loc[Config.Lan]["common"].current,
                event = "jim-mechanic:client:Tires:Apply", args = { R = v.r, G = v.g, B = v.b },
            }
        end
