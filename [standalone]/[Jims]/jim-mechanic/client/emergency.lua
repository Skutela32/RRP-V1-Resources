...
RegisterNetEvent('jim-mechanic:client:Emergency:Check', function(part) local validMods, Menu, vehicle, Ped, coords = {}, {}, nil, PlayerPedId(), GetEntityCoords(PlayerPedId())
    ...
    if Config.Emergency.LockEmergency then if getClass(vehicle) ~= "Emergency" then triggerNotify(nil, Loc[Config.Lan]["police"].restrict, "error") return end end
	if DoesEntityExist(vehicle) then
        if Config.System.Menu == "qb" then
            Menu[#Menu+1] = { icon = part, header = Loc[Config.Lan]["police"].header, txt = Loc[Config.Lan]["check"].label10, isMenuHeader = true }
        ...
        if Config.System.Debug then
            Menu[#Menu+1] = {
                icon = "fas fa-bug",
                header = "Vehicle Death (Debug)", params = { event = "jim-mechanic:client:Emergency:Damage" },
                title = "Vehicle Death (Debug)", event = "jim-mechanic:client:Emergency:Damage"
            }
        end
        ...
        if hideRepair ~= true then
            if Config.Emergency.CosmeticTable["Repair"] then
                Menu[#Menu+1] = {
                    icon = "fas fa-wrench",
                    header = Loc[Config.Lan]["police"].repair, params = { event = "jim-mechanic:client:Emergency:Repair", },
                    title = Loc[Config.Lan]["police"].repair, event = "jim-mechanic:client:Emergency:Repair"
                }
            end
        end
        if Config.Emergency.CosmeticTable["Paints"] then
            Menu[#Menu+1] = {
                ...
                header = Loc[Config.Lan]["paint"].menuheader, params = { event = "jim-mechanic:client:Emergency:Paints", },
                title = Loc[Config.Lan]["paint"].menuheader, event = "jim-mechanic:client:Emergency:Paints"
            }
        end
        if Config.Emergency.PreformaceTable["Engine"] then local id = 11
            local installed = "LVL: "..(GetVehicleMod(vehicle, id)+1 ~= 0 and GetVehicleMod(vehicle, id)+1 or Loc[Config.Lan]["common"].stock)
            Menu[#Menu+1] = {
                header = Loc[Config.Lan]["repair"].engine, txt = "["..(GetNumVehicleMods(vehicle, id)+1)..Loc[Config.Lan]["common"].menuinstalled..installed,
                params = { event = "jim-mechanic:client:Emergency:Choose", args = { perform = true, id = id, header = Loc[Config.Lan]["repair"].engine } },
                title = Loc[Config.Lan]["repair"].engine, description = "["..(GetNumVehicleMods(vehicle, id)+1)..Loc[Config.Lan]["common"].menuinstalled..installed,
                event = "jim-mechanic:client:Emergency:Choose", args = { perform = true, id = id, header = Loc[Config.Lan]["repair"].engine },
            }
        end
        if Config.Emergency.PreformaceTable["Brakes"] then
            ...
            Menu[#Menu+1] = {
                ...
                params = { event = "jim-mechanic:client:Emergency:Choose", args = { perform = true, id = id, header = label } },
                title = label, description = "["..(GetNumVehicleMods(vehicle, id)+1)..Loc[Config.Lan]["common"].menuinstalled..installed,
                event = "jim-mechanic:client:Emergency:Choose", args = { perform = true, id = id, header = label },
            }
        end
        ...
        if Config.Emergency.PreformaceTable["Armour"] then
            ...
            if GetNumVehicleMods(vehicle, id) ~= 0 then
                ...
                Menu[#Menu+1] = {
                    ...
                    params = { event = "jim-mechanic:client:Emergency:Toggles", args = { id = id, header = label } },
                    title = label, description = installed,
                    event = "jim-mechanic:client:Emergency:Toggles", args = { id = id, header = label },
                }
            end
        end
        ...
        for k, v in pairs(cosmeticTable) do
            ...
            if (v.roofLiv and GetVehicleRoofLiveryCount(vehicle) >= 1) or (v.oldLiv and GetVehicleLiveryCount(vehicle) >= 1) or v.plate or v.extra or v.window or v.horn then local txt, canDo, extraCount = "", true, 0
                ...
                if canDo then
                    Menu[#Menu+1] = { header = v.header, txt = txt, params = { event = "jim-mechanic:client:Emergency:Choose", args = v }, title = v.header, description = txt, event = "jim-mechanic:client:Emergency:Choose", args = v, }
                end
            end
            if v.enable and GetNumVehicleMods(vehicle, v.id) >= 1 then
                ...
                Menu[#Menu+1] = {
                    ...
                    params = { event = "jim-mechanic:client:Emergency:Choose", args = v },
                    title = v.header, description = "["..(GetNumVehicleMods(vehicle, v.id)+1)..Loc[Config.Lan]["common"].menuinstalled..installed,
                    event = "jim-mechanic:client:Emergency:Choose", args = v,
                }
            end

        end
		if Config.System.Menu == "ox" and countTable(Menu) ~= 0 then exports.ox_lib:registerContext({id = 'EmergencyMenu', title = Loc[Config.Lan]["police"].header, position = 'top-right', options = Menu }) exports.ox_lib:showContext("EmergencyMenu")
		elseif Config.System.Menu == "qb" and countTable(Menu) ~= 2 then exports['qb-menu']:openMenu(Menu)
        else
...
RegisterNetEvent('jim-mechanic:client:Emergency:Choose', function(data) local validMods, Menu, vehicle, Ped, coords = {}, {}, nil, PlayerPedId(), GetEntityCoords(PlayerPedId())
	if not IsPedInAnyVehicle(playerPed, false) then vehicle = getClosest(coords) pushVehicle(vehicle) else vehicle = GetVehiclePedIsIn(playerPed, false) pushVehicle(vehicle) end
	...
	if DoesEntityExist(vehicle) then
        ...
        Menu[#Menu+1] = {
            isMenuHeader = true, disabled = (Config.System.Menu == "ox"),
            header = Loc[Config.Lan]["police"].header, txt = Loc[Config.Lan]["common"].amountoption..#validMods+1,
            title = data.header, txt = Loc[Config.Lan]["common"].amountoption..#validMods+1,
        }
		Menu[#Menu+1] = {
			icon = "fas fa-circle-arrow-left",
			header = "", txt = Loc[Config.Lan]["common"].ret, params = { event = "jim-mechanic:client:Emergency:Check", args = data },
			title = Loc[Config.Lan]["common"].ret, event = "jim-mechanic:client:Emergency:Check"
		}
        if not data.horn and not data.extra and not data.oldLiv then
            ...
            Menu[#Menu+1] = {
                ...
                header = "0. "..Loc[Config.Lan]["common"].stock, txt = stockinstall, params = { event = "jim-mechanic:client:Emergency:Apply",
                args = { mod = -1, header = data.header, id = data.id, plate = data.plate, window = data.window, oldLiv = data.oldLiv, roofLiv = data.roofLiv, extra = data.extra, perform = data.perform }},
                title = "0 - "..Loc[Config.Lan]["common"].stock, description = stockinstall, event = "jim-mechanic:client:Emergency:Apply",
                args = { mod = -1, header = data.header, id = data.id, plate = data.plate, window = data.window, oldLiv = data.oldLiv, roofLiv = data.roofLiv, extra = data.extra, perform = data.perform },
            }
        elseif data.horn then
            data.veh = vehicle
            Menu[#Menu+1] = {
                header = "Test Horn", params = { event = "jim-mechanic:client:Emergency:Horn:Test", args = data, },
                title = "Test Horn", event = "jim-mechanic:client:Emergency:Horn:Test", args = data,
            }
        end
		for l, b in pairsByKeys(validMods) do
			Menu[#Menu+1] = {
				...
				params = { event = "jim-mechanic:client:Emergency:Apply", args = b },
				title = l.." - "..b.name, description = b.install,
				event = "jim-mechanic:client:Emergency:Apply", args = b
			}
		end
	...
	if data.id and GetVehicleMod(vehicle, tonumber(data.id)) == tonumber(data.mod) then
		...
		if progressBar({label = Loc[Config.Lan]["common"].installing..modName, time = 1000, cancel = true }) then
            ...
            elseif data.extra then
                ...
                doCarDamage(vehicle, veh)                Wait(100)
...
RegisterNetEvent('jim-mechanic:client:Emergency:Horn:Test', function(data)
...
RegisterNetEvent('jim-mechanic:client:Emergency:Paints', function() local Ped = PlayerPedId() local vehicle = nil
    ...
    local PaintMenu = {}
    if Config.System.Menu == "qb" then
        PaintMenu[#PaintMenu + 1] = { icon = "paintcan", header = Loc[Config.Lan]["paint"].menuheader, txt = "", isMenuHeader = true }
    end
    PaintMenu[#PaintMenu+1] = { icon = "fas fa-circle-arrow-left", header = "", txt = Loc[Config.Lan]["common"].ret, title = Loc[Config.Lan]["common"].ret,
            params = { event = "jim-mechanic:client:Emergency:Check" }, event = "jim-mechanic:client:Emergency:Check" }
    PaintMenu[#PaintMenu + 1] = {
        header = Loc[Config.Lan]["paint"].primary, txt = Loc[Config.Lan]["common"].current..": "..vehPrimaryColour,
        params = { event = "jim-mechanic:client:Emergency:Paints:Choose", args = Loc[Config.Lan]["paint"].primary },
        title = Loc[Config.Lan]["paint"].primary, description = Loc[Config.Lan]["common"].current..": "..vehPrimaryColour,
        event = "jim-mechanic:client:Emergency:Paints:Choose", args = Loc[Config.Lan]["paint"].primary
    }
    PaintMenu[#PaintMenu + 1] = {
        header = Loc[Config.Lan]["paint"].secondary, txt = Loc[Config.Lan]["common"].current..": "..vehSecondaryColour,
        params = { event = "jim-mechanic:client:Emergency:Paints:Choose", args = Loc[Config.Lan]["paint"].secondary },
        title = Loc[Config.Lan]["paint"].secondary, description = Loc[Config.Lan]["common"].current..": "..vehSecondaryColour,
        event = "jim-mechanic:client:Emergency:Paints:Choose", args = Loc[Config.Lan]["paint"].secondary
    }
    PaintMenu[#PaintMenu + 1] = {
        header = Loc[Config.Lan]["paint"].pearl, txt = Loc[Config.Lan]["common"].current..": "..vehPearlescentColour,
        params = { event = "jim-mechanic:client:Emergency:Paints:Choose", args = Loc[Config.Lan]["paint"].pearl },
        title = Loc[Config.Lan]["paint"].pearl, description = Loc[Config.Lan]["common"].current..": "..vehPearlescentColour,
        event = "jim-mechanic:client:Emergency:Paints:Choose", args = Loc[Config.Lan]["paint"].pearl
    }
    PaintMenu[#PaintMenu + 1] = {
        header = Loc[Config.Lan]["paint"].wheel, txt = Loc[Config.Lan]["common"].current..": "..vehWheelColour,
        params = { event = "jim-mechanic:client:Emergency:Paints:Choose", args = Loc[Config.Lan]["paint"].wheel },
        title = Loc[Config.Lan]["paint"].wheel, description = Loc[Config.Lan]["common"].current..": "..vehWheelColour,
        event = "jim-mechanic:client:Emergency:Paints:Choose", args = Loc[Config.Lan]["paint"].wheel
    }
...
RegisterNetEvent('jim-mechanic:client:Emergency:Paints:Choose', function(data) local Ped = PlayerPedId() local vehicle = nil
	...
	if DoesEntityExist(vehicle) then
		local Menu = {}
		if Config.System.Menu == "qb" then
			Menu[#Menu+1] = { icon = "fas fa-brush", header = data..Loc[Config.Lan]["paint"].menuheader, txt = "", isMenuHeader = true }
		end
		Menu[#Menu+1] = { icon = "fas fa-circle-arrow-left", header = "", txt = Loc[Config.Lan]["common"].ret, title = Loc[Config.Lan]["common"].ret,
			params = { event = "jim-mechanic:client:Emergency:Paints" }, event = "jim-mechanic:client:Emergency:Paints" }

		Menu[#Menu+1] = { header = Loc[Config.Lan]["paint"].metallic, title = Loc[Config.Lan]["paint"].metallic,
			params = { event = "jim-mechanic:client:Emergency:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].metallic } },
			event = "jim-mechanic:client:Emergency:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].metallic } }

		Menu[#Menu+1] = { header = Loc[Config.Lan]["paint"].matte, title = Loc[Config.Lan]["paint"].matte,
			params = { event = "jim-mechanic:client:Emergency:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].matte } },
					event = "jim-mechanic:client:Emergency:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].matte } }

		Menu[#Menu+1] = { header = Loc[Config.Lan]["paint"].metals, title = Loc[Config.Lan]["paint"].metals,
					params = { event = "jim-mechanic:client:Emergency:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].metals } },
					event = "jim-mechanic:client:Emergency:Paints:Choose:Colour", args = { paint = data, finish = Loc[Config.Lan]["paint"].metals } }

        if Config.System.Menu == "ox" then exports.ox_lib:registerContext({id = 'Menu', title = data..Loc[Config.Lan]["paint"].menuheader, position = 'top-right', options = Menu }) exports.ox_lib:showContext("Menu")
        elseif Config.System.Menu == "qb" then exports['qb-menu']:openMenu(Menu) end
	end
end)
...
RegisterNetEvent('jim-mechanic:client:Emergency:Paints:Choose:Colour', function(data) local Ped = PlayerPedId() local vehicle = nil
	...
	local PaintMenu = {}
	if Config.System.Menu == "qb" then
		PaintMenu[#PaintMenu+1] = { icon = "fas fa-brush", isMenuHeader = true, header = data.finish.." "..data.paint }
	end
	PaintMenu[#PaintMenu+1] = { icon = "fas fa-circle-arrow-left", header = "", txt = Loc[Config.Lan]["common"].ret, title = Loc[Config.Lan]["common"].ret,
		params = { event = "jim-mechanic:client:Emergency:Paints:Choose", args = data.paint },
		event 	= "jim-mechanic:client:Emergency:Paints:Choose", args = data.paint }
	local paintTable = {
	...
	for k, v in pairs(paintTable[data.finish]) do local current = (colourCheck == v.id)
		PaintMenu[#PaintMenu+1] = { icon = curtent and "fas fa-check", isMenuHeader = current, disabled = (Config.System.Menu == "ox" and current),
			header = k..". "..v.name, txt = current and Loc[Config.Lan]["common"].current, title = k.." - "..v.name, description = curent and Loc[Cnfig.Lan]["common"].current,
			params = { event = 'jim-mechanic:client:Emergency:Paints:Apply', args = { paint = data.paint, id = v.id, name = v.name, finish = data.finish } },
			event = 'jim-mechanic:client:Emergency:Paints:Apply', args = { paint = data.paint, id = v.id, name = v.name, finish = data.finish } }
	end
