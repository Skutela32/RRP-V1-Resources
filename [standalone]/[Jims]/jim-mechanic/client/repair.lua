...
RegisterNetEvent('jim-mechanic:client:Repair:Check', function(skip) local Ped = PlayerPedId()
	...
	local RepairMenu = {}
	local br = Config.System.Menu == "ox" and "\n" or "<br>"
	local header = Loc[Config.Lan]["check"].plate.." ["..plate.."]"..br..Loc[Config.Lan]["check"].value..searchPrice(vehicle)..br..searchDist(vehicle)
	RepairMenu[#RepairMenu + 1] = { isMenuHeader = true, disabled = (Config.System.Menu == "ox"), header = searchCar(vehicle), txt = header, title = header, }
	if Config.System.Menu == "qb" then
		RepairMenu[#RepairMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan]["common"].close, params = { event = "jim-mechanic:client:Menu:Close" } }
	end
	local mechjobTable = {}
	...
	for _, v in pairs(mechjobTable) do
		...
		RepairMenu[#RepairMenu+1] =	{
			icon = v.icon,
			isMenuHeader = (headerlock or skip), disabled = ((Config.System.Menu == "ox" and headerlock) or (Config.System.Menu == "ox" and skip)),
			header = v.header, params = { event = "jim-mechanic:client:Repair:Sure", args = { part = v.part, partname = v.header, vehicle = vehicle, cost = v.amount } },
			title = v.header.." - "..math.floor((damageTable[v.part]+0.5)).."%", description = costTable[v.part],
			event = "jim-mechanic:client:Repair:Sure", args = { part = v.part, partname = v.header, vehicle = vehicle, cost = v.amount },
			progress = damageTable[v.part],
			colorScheme = not headerlock and ((damageTable[v.part] and (damageTable[v.part] > 80)) and "green" or ((damageTable[v.part] and (damageTable[v.part] < 30)) and "red") or "yellow") or "grey"
		}
		...
		else RepairMenu[#RepairMenu].text = nosBar(math.floor((damageTable[v.part]+0.5) or 90)) .." "..math.floor((damageTable[v.part]+0.5) or 90).."%<br>"..(costTable[v.part] or damageTable["wheels"]) end
	end
...
RegisterNetEvent('jim-mechanic:client:Repair:Sure', function(data)
	...
	if lockedCar(vehicle) then return end
	if DoesEntityExist(vehicle) then
		local header = Loc[Config.Lan]["check"].plate.." ["..trim(GetVehicleNumberPlateText(vehicle)).."]"..br..Loc[Config.Lan]["check"].value..searchPrice(vehicle)..br..searchDist(vehicle)
		RepairMenu[#RepairMenu+1] = { isMenuHeader = true, disabled = Config.System.Menu == "ox", header = searchCar(vehicle), txt = header, title = header }
		RepairMenu[#RepairMenu+1] = { header = Loc[Config.Lan]["repair"].doyou..data.partname.."?", isMenuHeader = true, title = Loc[Config.Lan]["repair"].doyou..data.partname.."?", disabled = (Config.System.Menu == "ox") }
