...
RegisterNetEvent('jim-mechanic:client:Menu', function(editable) local ped = PlayerPedId() emptyHands(ped)
	...
	if Config.System.Menu == "qb" then CheckMenu[#CheckMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan]["common"].close, params = { event = "jim-mechanic:client:Menu:Close" } } end
	local CheckTable = {
	...
	for k, v in ipairs(CheckTable) do
		if v.hide and v.lock then else
			...
			CheckMenu[#CheckMenu+1] = {
				...
				title = v.head, description = v.desc, event = "jim-mechanic:client:Menu:Remove", args = { vehicle = vehicle, mod = v.part, extra = extra }, progress = v.progress,
				colorScheme = editable and ((v.progress and (v.progress > 80)) and "green" or ((v.progress and (v.progress < 30)) and "red") or "yellow" ) or "grey"
			}
		end
	end
...
RegisterNetEvent('jim-mechanic:client:Menu:List', function() local Ped = PlayerPedId()
	...
	if Config.System.Menu == "qb" then
		CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = Loc[Config.Lan]["check"].label11..searchCar(vehicle), txt = Loc[Config.Lan]["check"].label10 }
		CheckMenu[#CheckMenu+1] = { icon = "fas fa-circle-xmark", header = "", txt = Loc[Config.Lan]["common"].close, params = { event = "jim-mechanic:client:Menu:Close" } }
	end
	local exTable = {
	...
	for k in pairs(exTable) do if GetNumVehicleMods(vehicle, v) ~= 0 then external = true break end end
	for k in pairs(inTable) do if GetNumVehicleMods(vehicle, v) ~= 0 then internal = true break end end
	...
	if external then
		...
		if GetNumVehicleMods(vehicle, 48) ~= 0 then
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, disabled = (Config.System.Menu == "ox"), header = "", txt = Loc[Config.Lan]["police"].livery.." - [ "..GetNumVehicleMods(vehicle, 48).." "..Loc[Config.Lan]["check"].label12.." ]",
				disabled = (Config.System.Menu == "ox"),title = Loc[Config.Lan]["police"].livery.." - [ "..GetNumVehicleMods(vehicle, 48).." "..Loc[Config.Lan]["check"].label12.." ]", }
		end
		if GetVehicleLiveryCount(vehicle) ~= -1 then
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true,  header = "", txt = Loc[Config.Lan]["police"].livery.." - [ "..GetVehicleLiveryCount(vehicle).." "..Loc[Config.Lan]["check"].label12.." ]",
				disabled = (Config.System.Menu == "ox"), title = Loc[Config.Lan]["police"].livery.." - [ "..GetVehicleLiveryCount(vehicle).." "..Loc[Config.Lan]["check"].label12.." ]", }
		end
		for k, v in pairs(exTable) do if GetNumVehicleMods(vehicle, k) ~= 0 then
				CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "", txt = v..GetNumVehicleMods(vehicle, k).." "..Loc[Config.Lan]["check"].label12.." ]",
				disabled = (Config.System.Menu == "ox"), title = v..GetNumVehicleMods(vehicle, k).." "..Loc[Config.Lan]["check"].label12.."]", }
			end end
	end
	if internal then
		CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = Loc[Config.Lan]["check"].label14, disabled = (Config.System.Menu == "ox"), title = Loc[Config.Lan]["check"].label14, }
		for k, v in pairs(inTable) do if GetNumVehicleMods(vehicle, k) ~= 0 then
			CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = "", txt = v..GetNumVehicleMods(vehicle, k).." "..Loc[Config.Lan]["check"].label12.." ]",
			disabled = (Config.System.Menu == "ox"), title = v..GetNumVehicleMods(vehicle, k).." "..Loc[Config.Lan]["check"].label12.."]", }
	end end end
	local mods = nil
...
RegisterNetEvent('jim-mechanic:client:Menu:Remove', function(data) local plate = trim(GetVehicleNumberPlateText(vehicle))
	...
	local CheckMenu = { }
	if Config.System.Menu == "qb" then
		CheckMenu[#CheckMenu+1] = { isMenuHeader = true, header = searchCar(data.vehicle), txt = Loc[Config.Lan]["check"].plate..trim(GetVehicleNumberPlateText(data.vehicle)).."<br>"..Loc[Config.Lan]["check"].value..searchPrice(data.vehicle).."<br>"..searchDist(data.vehicle) }
	end
